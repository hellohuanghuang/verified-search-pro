...
Verified Search Pro 是一套面向深度调研和事实核查的可信研究 Skill。当前公开版本是 **v2.1.0-beta**：在 v2.0.0 稳定版基础上新增腾讯云 WSA 与百度千帆两个 API 引擎（与 Tavily 组成 L1 主力，未配置自动跳过）、中文搜索优化、DuckDuckGo 引擎与降级、必应中文疑问词改写、搜狗链接解密、头条低频引擎、限制标注中文化、强制 LLM 概念提取、更强的 Tavily 询问机制和零第三方依赖保持。
