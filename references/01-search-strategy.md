...
| 场景 | 首选引擎 | 辅助引擎 | 原因 |
|------|---------|---------|------|
| **中文事实核查** | tencent_wsa（已配置时） | 必应 / 宿主搜索输入 | 腾讯云 WSA 中文覆盖好、返回发布日期；未配置时退回必应 + 可用百度 |
| **英文/国际信息** | tavily + 必应国际 | 必应 / 宿主搜索输入 | Tavily 英文质量高 |
| **科技/行业动态** | tavily / tencent_wsa / 宿主搜索输入 | 必应 / 宿主搜索输入 | API 引擎补时效与语义覆盖 |
| **微信公众号内容** | 搜狗微信 | 百度 / 宿主搜索输入 | 搜狗微信搜索专精；腾讯云 WSA 不含公众号内容 |
| **政策/政府信息** | tencent_wsa（已配置时）/ 百度 | 必应 / 宿主搜索输入 | WSA 可命中政府站点；百度对 gov.cn 收录好 |
| **通用快速查询** | tavily / tencent_wsa | 必应 / 宿主搜索输入 | API 引擎速度+质量平衡 |
| **观点/争议/评价** | 必应 + 可用百度 | tavily / 宿主搜索输入 | 先覆盖多元材料，再在输出端清晰标注 |

**API 引擎分工**：腾讯云 WSA（`tencent_wsa`）为中文 API 主力，覆盖腾讯系/搜狗内容生态，返回发布时间元数据，适合中文事实核查、政策追踪；但**不包含微信公众号内容**（官方 FAQ 明示），公众号需求仍走搜狗微信或宿主输入。Tavily 负责国际/英文向搜索，与 WSA 互补。两者均为 L1 可选增强，未配置时自动降级到免费 HTML 引擎，配置步骤见 `references/10-api-setup.md`。
